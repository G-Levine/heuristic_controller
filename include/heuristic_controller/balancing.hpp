...
#include "heuristic_controller/eigen-qp.hpp"

// Eigen::Matrix<double, 3, 4> balancingNaive(const Eigen::Matrix<double, 3, 4>& r,
...
Eigen::Matrix<double, 3, 4> balancingQP(const Eigen::Matrix<double, 3, 4>& r,
                                                   ...
                                                   double weight_f = 1.0, double weight_tau = 1.0, double weight_diff = 0.01, double weight_mag = 0.01) {
    // Check if no legs are in contact
    if (s.sum() == 0) {
        return Eigen::Matrix<double, 3, 4>::Zero();
    }
